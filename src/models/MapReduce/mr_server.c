...
tw_peid
mr_server_mapping(tw_lpid gid)
{
  return (tw_peid) gid / g_tw_nlp;
}

void
mr_server_init(mr_server_state * s, tw_lp * lp)
{
  int i,j,flag;
  tw_event *e;
  mr_server_message *m;
...
void
mr_server_event_handler(mr_server_state * s, tw_bf * bf, mr_server_message * msg, tw_lp * lp)
{
  int rand_result, i;
  tw_lpid dest_lp;
  tw_stime ts;
  ...
  switch(msg->msg_core.type)
    ...
    case VS_MR_SERVER_WRITE_SET_UP:
      {
	...
	if (lp->gid == s->namenode_id)
	  {
	    ...
	    m->msg_core.type = VS_MR_SERVER_WRITE_SET_UP_ACK;
	    tw_event_send(e);	    
	  }
	else
    ...
    case VS_MR_SERVER_WRITE_SET_UP_ACK:
      {
	...
	//m->msg_core.type = VS_MR_SERVER_WRITE_DATA_SEND;
	m->msg_core.type = VS_MR_SERVER_WRITE_SOCKET_SET_UP;
    ...
    case VS_MR_SERVER_WRITE_SOCKET_SET_UP:
      {
	...
	if (msg->msg_core.replica_counter<N_REPLICA)
	...
	else
	  {
	    ...
	    m = tw_event_data(e);
	    m->msg_core = msg->msg_core;
	    m->msg_core.type = VS_MR_SERVER_WRITE_SOCKET_SET_UP_ACK;
	    tw_event_send(e);
	  }
	break;
      }

    case VS_MR_SERVER_WRITE_SOCKET_SET_UP_ACK:
      {
    ...
    case VS_MR_SERVER_WRITE_DATA_SEND:
      {
	...
	if (s->pkt_send_counter < Write_Request_size/Pkt_size)
	  {
	    ...
	    e = tw_event_new(s->data_node_ID[0], Pkt_size/Buffer_Copy_rate, lp);
            ...
            m->msg_core.type = VS_MR_SERVER_WRITE_DATA_SEND_ACK;
	    m->msg_core.src_pid[0] = lp->gid;
            tw_event_send(e);
	  }
    ...
    case VS_MR_SERVER_WRITE_DATA_SEND_ACK:
      {
	// split request to packet
	...
	m->msg_core.type = VS_MR_SERVER_WRITE_DONE;
	tw_event_send(e);

	break;
      }

    case VS_MR_SERVER_WRITE_DONE:
      {
	s->pkt_recv_counter++;
	if ( s->pkt_recv_counter == Write_Request_size/Pkt_size )
	  {	    
	    ...
	    m->msg_core = msg->msg_core;
	    push_src(&(m->msg_core),&lp->gid);
	    m->msg_core.type = VS_MR_SERVER_WRITE_CLOSE;
	    tw_event_send(e);
	  }
	else
	  printf("\tMessage is not at name node, Please check!\n");
	...
	if (lp->gid == s->namenode_id)
	  {
	    ...
	    m->msg_core = msg->msg_core;
	    m->msg_core.type = VS_MR_SERVER_WRITE_CLOSE_ACK;
	    tw_event_send(e);
	  }
	break;
      }

    case VS_MR_SERVER_WRITE_CLOSE_ACK:
      {	    
	printf("Close finished at %d\n",lp->gid);
...
void
mr_server_rc_event_handler(mr_server_state * s, tw_bf * bf, mr_server_message * msg, tw_lp * lp)
{
  switch(msg->msg_core.type)
  {
    /* case ARRIVAL: */
...
void
mr_server_final(mr_server_state * s, tw_lp * lp)
{
  //fclose(s->logfile);
}
